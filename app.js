...
async function renderHome(){
  ...
  if(avRow)avRow.innerHTML=window.chicas.map(c=>`<div class="av" style="background:${c.bg_color};color:${c.color};">${(c.apodo||c.nombre).slice(0,2)}</div>`).join('');
...
function renderEventos(f){
  ...
  container.innerHTML=filtered.map(e=>{
    ...
    return `<div class="evento-card"><div style="height:3px;background:${t.color};"></div><div class="evento-card-header"><div style="flex:1;"><div style="margin-bottom:5px;"><span class="badge ${t.badge}">${t.icon} ${t.label}</span></div><div class="evento-nombre">${e.nombre}</div><div class="evento-meta"><span>${fStr}</span>${e.hotel||e.lugar?`<span>📍 ${e.hotel||e.lugar}</span>`:''} ${e.confirmadas_count?`<span>👯 ${e.confirmadas_count} van</span>`:''}</div></div><button class="icon-btn e" onclick="openEditEventoModal(${e.id})"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button></div><div class="evento-actions">${e.tipo==='viaje'?`<button class="evento-btn primary" onclick="abrirDetalle(${e.id})">Ver itinerario →</button>`:''}<button class="evento-btn" onclick="abrirEncuestas(${e.id})">🗳️ Encuestas</button><button class="evento-btn" onclick="shareWhatsApp('evento',${e.id})">📱 Compartir</button></div></div>`;
  }).join('');
}
...
function renderFinanzasAll(){renderFinStats();renderFinGastos();renderFinDeudas();renderFinBalance();}
...
function onComprobanteSelected(input) {
  ...
  if (file.type.startsWith('image/') && preview) {
    ...
    preview.innerHTML = `<img src="${url}" style="width:100%;border-radius:8px;max-height:120px;object-fit:cover;">`;
  } else if (preview) {
    preview.innerHTML = `<div style="background:var(--aurora-l);border-radius:8px;padding:8px 10px;font-size:11px;color:var(--aurora-d);">📄 ${file.name}</div>`;
  }
}
...
function quitarComprobante() {
  ...
  // re-render modal without comprobante
  const prefill = finState.editingGasto ? {...finState.editingGasto, comprobante_url: null, comprobante_name: null} : null;
  _openGastoModal(prefill);
}
