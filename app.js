...
async function saveGastoAPI(){const nombre=document.getElementById('fg-nombre').value.trim();const monto=parseFloat(document.getElementById('fg-monto').value);const pagadoPor=parseInt(document.getElementById('fg-pagadopor').value);const soloReg=document.getElementById('fg-solo')?.classList.contains('on')||false;if(!nombre||!monto||monto<=0)return;if(!soloReg&&!finState.selSplit.length)return;const payload={evento_id:finState.eventoId,nombre,monto,moneda:finState.selMon,categoria:finState.selCats.join(','),pagado_por:pagadoPor,participantes:soloReg?[]:finState.selSplit,notas:document.getElementById('fg-notas').value.trim(),fecha_registro:document.getElementById('fg-fecha')?.value||null,solo_registro:soloReg};if(finState.editingGasto)await put(`/gastos/${finState.editingGasto.id}`,payload);else await post('/gastos',payload);closeModal();await Promise.all([refreshFinanzas(),renderHome()]);}
